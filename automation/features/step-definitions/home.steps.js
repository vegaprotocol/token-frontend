...
Given(/^I have not connected my eth wallet$/,  () => {
   HomePage.disconnectWalletBtn.click();
});
...
Then(
  /^the error message is displayed "([^"]*)?" on page$/,
   (expectedError) => {
...
Then(/^I can see the token address is shown$/,  () => {
  ...
  expect( tokenAddress.getText()).toBe(
    "0x547cbA83a7eb82b546ee5C7ff0527F258Ba4546D"
  );
});

Then(/^the vesting address is shown$/,  () => {
  const vestingAddress =  browser.getByTestId(
    HomePage.vestingAddressTestId
  );
...
Then(/^the total supply is shown correctly$/,  () => {
  ...
  let totalSupplyNum =  Number(
    ( totalSupply.getText()).replace(/,/g, "")
  );
...
Then(
  /^I can see the vega wallet disconnected with message "([^"]*)?"$/,
   (disconnectedMsg) => {
    const ConnectVegaTxt =  (
       browser.getByTestId(HomePage.connectVegaWalletTestId)
    ).getText();
