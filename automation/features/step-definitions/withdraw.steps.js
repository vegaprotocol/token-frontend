...
Then(
  /^I can see the wallet address field is showing "([^"]*)?"$/,
  (walletAddress) => {
...
When(/^I click the link to enter a new wallet address$/, () => {
  withdrawalsPage.enableWalletAddressFieldLink.waitForDisplayed({
    timeout: 10000,
    timeoutMsg: "Enter wallet address manually link not displayed",
  });
  expect(withdrawalsPage.enableWalletAddressFieldLink).toHaveText(
...
Then(
  /^the wallet address link is changed to "([^"]*)?"$/,
  (walletAddresslinkText) => {
    expect(withdrawalsPage.enableWalletAddressFieldLink).toHaveText(
      walletAddresslinkText
    );
  }
...
Then(
  /^I can see the wallet address in the input field is "([^"]*)?"$/,
  (newWalletAddress) => {
