...
When(/^I can see the pending transactions button is shown$/, async () => {
  browser.getByTestId("pending-transactions-btn").waitForDisplayed({
    timeout: 20000,
    reverse: false,
    timeoutMsg: "Pending transactions button was not found",
  });

  await expect(browser.getByTestId("pending-transactions-btn")).toHaveText(
...
When(/^I click on use maximum button$/, () => {
  ...
  if (browser.getByTestId("associate-more-tokens-btn").isDisplayed()) {
    ...
    browser.getByTestId("associate-tokens-btn").click();
  }
  $('[data-testid="associate-radio-wallet"]').click({ force: true });
