class Page {
    ...
    confirmTransactionSpeedUp() {
        ...
        $('.transaction-detail-edit').waitForDisplayed({ timeout: 10000, timeoutMsg: "Edit gas fee link did not display in 10 seconds" })
        $('button=Edit').click()
        $('.edit-gas-display__content').waitForDisplayed({ timeout: 10000, timeoutMsg: "Edit gas fee modal did not display in 10 seconds" })
        $('input[value="high"]').click()
        $('button=Save').waitForClickable({ timeout: 10000, timeoutMsg: "save button not clickable after 10 seconds" })
        $('button=Save').click()
        $('.edit-gas-display__content').waitForDisplayed({ timeout: 10000, reverse: true, timeoutMsg: "Edit gas fee modal did not disappear in 10 seconds" })
