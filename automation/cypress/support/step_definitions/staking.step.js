...
Then("I can see the number in the field is {string}",(amountOfStakeToRemove)=>{
    cy.get('[data-testid="token-amount-input"]').should('have.value',amountOfStakeToRemove)
})

Then("the remove button is now enabled again with message {string}",(submitBtnText)=>{
    cy.get('[data-testid="token-input-submit-button"]').should('be.enabled').and('have.text',submitBtnText)
})
...
cy.get('.callout__title').should('have.text',`Removing 100 $VEGA from node ${nodeName}`)
})

Then('next epoch credit message is displayed with message {string}',(nextEpochCreditMsg)=>{
    cy.get('.callout__title').siblings('p').should('have.text',nextEpochCreditMsg)
