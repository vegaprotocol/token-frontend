...
export const Web3Container = ({
      ...
      provider.on("accountsChanged", async (accounts: string[]) => {
        if (accounts.length) {
          const [balance, walletBalance, lien, allowance] = await Promise.all([
            ...
            token.allowance(accounts[0], ADDRESSES.stakingBridge),
          ]);
          appDispatch({
  ...
  if (APP_CHAIN_ID !== appState.chainId) {
    return (
      <Callout
        intent="error"
        icon={<Error />}
      ...
      >
        <p>
          {t("Desired network", {
            chain: EthereumChainNames[APP_CHAIN_ID],
          })}
        </p>
      </Callout>
    );
  ...
  if (appState.ethWalletConnecting) {
    return (
      <Callout>
  ...
  return children(appState.ethAddress);
};
